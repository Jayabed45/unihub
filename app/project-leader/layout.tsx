        ...
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true">
          ...
          >
            <div className="px-5 pt-6 pb-4">
              ...
              <p className="mt-0.5 text-xs text-gray-500">Project Leader</p>
            </div>
            <nav className="px-3 space-y-1">
              {projectLeaderNavigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  onClick={() => setMobileNavOpen(false)}
            ...
            <div className="p-4 mt-auto text-xs text-gray-400">© {new Date().getFullYear()} UniHub</div>
          </div>
        </div>
      )}
