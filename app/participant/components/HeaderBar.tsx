...
interface ParticipantHeaderBarProps {
  onToggleNotifications?: () => void;
  notificationsCount?: number;
  notificationsOpen?: boolean;
}

export default function ParticipantHeaderBar({
  onToggleNotifications,
  notificationsCount = 0,
  notificationsOpen,
}: ParticipantHeaderBarProps) {
  ...
  const title = useMemo(() => {
    const current = participantNavigation.find((item) => pathname === item.href || pathname?.startsWith(`${item.href}/`));
    return current?.name ?? 'Participant Workspace';
  }, [pathname]);

  const description = useMemo(() => {
    const current = participantNavigation.find((item) => pathname === item.href || pathname?.startsWith(`${item.href}/`));
    return current?.description ?? 'Stay updated on your extension projects.';
  }, [pathname]);
    ...
    <header className="sticky top-0 z-30 border-b border-amber-100 bg-white/80 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-4">
        ...
        <div className="hidden items-center gap-6 text-sm lg:flex">
          <label className="relative w-72 max-w-xs">
