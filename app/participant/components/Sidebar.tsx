...
import type { ParticipantNavItem } from '../navigation';
...
export default function ParticipantSidebar({ items, onLogout, logoutDisabled }: SidebarProps) {
  const pathname = usePathname();

  return (
    <aside className="hidden w-72 flex-col border-r border-amber-100 bg-white/80 backdrop-blur lg:flex">
      ...
      <div className="px-6 py-6 text-xs text-gray-400">
        ...
        <p className="mt-4">© {new Date().getFullYear()} UniHub Participants</p>
      </div>
    </aside>
  );
}
